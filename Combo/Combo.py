...
def createCombo(ComboWindow, inputFolder, outputFile):
    ...
    for fileInput in os.listdir(inputFolder):
        if fileInput.endswith(".mp4"):
            input_files.append(os.path.join(inputFolder, fileInput))
    ...
    final_clip.write_videofile(os.path.join(inputFolder, outputFile), fps=fps)
